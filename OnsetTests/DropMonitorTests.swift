...
struct DropMonitorCounterSeparationTests {
    ...
    func nonBackpressureDrops_stayNormal() async {
        ...
        continuation.yield(DropEvent(reason: .captureDrop, count: 4, detectedAt: pts))
        continuation.yield(DropEvent(reason: .cfrNormalizationDrops, count: 7, detectedAt: pts))
        continuation.finish()
...
struct DropMonitorTransitionTests {
    ...
    func normalDegradedNormal() async {
        ...
        for idx in 0..<(threshold + 1) {
            ...
            continuation.yield(DropEvent(reason: .encoderBackpressureDrops, count: 1, detectedAt: detectedAt))
        }
        continuation.finish()
    ...
    func batchedDropEvent_degradesActor() async {
        ...
        continuation.yield(DropEvent(reason: .encoderBackpressureDrops, count: threshold + 1, detectedAt: detectedAt))
        continuation.finish()
