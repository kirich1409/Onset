...
struct RecordingConfigurationTests {
    ...
    func mvpDefault_profileLevel_mainAutoLevel() {
        #expect(sut.profileLevel == .mainAutoLevel)
    }
    ...
    func mvpDefault_pixelFormatPreference() {
        #expect(sut.pixelFormatPreference == [.biPlanar420v, .biPlanar420f])
    }
    ...
    func mvpDefault_allowFrameReordering_true() {
        #expect(sut.allowFrameReordering == true)
    }
    ...
    func mvpDefault_movieFragmentInterval_4s() {
        #expect(sut.movieFragmentInterval == 4.0)
    }
    ...
    func bitrateLookup_4K60_positive() {
        let bitrate = sut.averageBitrate(forWidth: 3_840, height: 2_160, fps: 60)
        #expect(bitrate > 0)
    }

    @Test("averageBitrate — 1080p@30 returns a positive value from the table")
    func bitrateLookup_1080p30_positive() {
        let bitrate = sut.averageBitrate(forWidth: 1_920, height: 1_080, fps: 30)
        #expect(bitrate > 0)
    }
    ...
    func bitrateLookup_unknownKey_positiveFallback() {
        ...
        let bitrate = sut.averageBitrate(forWidth: 1_280, height: 720, fps: 24)
        #expect(bitrate > 0)
    }

    @Test("averageBitrate — 4K@60 bitrate is higher than 1080p@30 bitrate")
    func bitrateLookup_4K60_greaterThan_1080p30() {
        let bitrate4K60 = sut.averageBitrate(forWidth: 3_840, height: 2_160, fps: 60)
        let bitrate1080p30 = sut.averageBitrate(forWidth: 1_920, height: 1_080, fps: 30)
    ...
    func mvpDefault_outputDirectory_endsWithMoviesOnset() {
        let components = sut.outputDirectory.pathComponents
    ...
    func budgetCap_value_995M() {
        #expect(sut.budgetCap.maxPixelsPerSecond == 995_000_000)
    }
    ...
    func budgetCap_4K60_1080p30_fits() {
        ...
        let fits = sut.budgetCap.fits(
            screen: SourceDimensions(width: 3_840, height: 2_160, fps: 60),
            camera: SourceDimensions(width: 1_920, height: 1_080, fps: 30)
        )
        #expect(fits)
    }
    ...
    func budgetCap_8K60_exceeds() {
        ...
        let fits = sut.budgetCap.fits(
            screen: SourceDimensions(width: 7_680, height: 4_320, fps: 60),
    ...
    func budgetCap_5K60_plus_4K60_exceeds() {
        ...
        let fits = sut.budgetCap.fits(
            screen: SourceDimensions(width: 5_120, height: 2_880, fps: 60),
            camera: SourceDimensions(width: 3_840, height: 2_160, fps: 60)
        )
        #expect(!fits)
    }
}
