...
| Steps | 1. Запустить Onset.app с камерой (встроенная или внешняя). 2. Наблюдать превью камеры в главном окне. 3. Визуально проверить: превью не искажено (изображение не сжато и не растянуто); соотношение сторон видимо ≈16:9 (не квадратное, не 4:3). 4. Измерить высоту превью: ≤180pt (не выходит за секцию). 5. Убедиться: кнопка «Записать» видна без прокрутки при стандартном размере окна 460×660pt. 6. (Опционально) Подключить вторую камеру с иным native AR → превью адаптируется к 16:9 без деформации (letterbox/pillarbox). |
