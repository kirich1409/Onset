...
недостижим (Brio hardware-constraint, [#178](https://github.com/kirich1409/Onset/issues/178)); добавлен
§3.7 критические сигналы записи (3 условия «пожара», таблица 9 порогов, заметка о L5-калибровке на MX
Brio по AC-7); ранее (v0.4): §3.6 покрытие кода (L2, информационно), ярусная матрица устройств
(Tier 0 / Tier 1), встроенный дисплей MacBook, graceful-поведение при отсутствии внешних устройств
...
### 3.7 Критические сигналы записи

Активные сигналы пользователю о «пожаре» во время записи (меню-бар-октагон + системное
уведомление) сверх дискового пути для мелких дропов (#246). Реализовано и покрыто L2
(чистые детекторы + state-machine координатора); пороги ещё **не откалиброваны на L5**.

**Три условия «пожара»** (`CriticalIncident`):

| Условие | Когда срабатывает | Severity |
|---|---|---|
| Потеря камеры (`cameraLost`) | Камера отключена во время записи. `cameraOnly` — камера была единственным видеоисточником, сессия остановлена (hard); `cameraAndScreen` — экран продолжает запись (soft) | hard / soft |
| Устойчивые потери (`sustainedDrops`) | LIVE: `.degraded` держится непрерывно ≥ `criticalSustainSeconds`. POST-STOP: нормализованная интенсивность drops/мин ≥ `criticalDropRatePerMin` при длительности ≥ `criticalDropRateMinSessionSeconds` | hard |
| Коллапс fps камеры (`fpsCollapse`) | delivered-fps держится < `fpsCollapseRatio × baseline` ≥ `fpsCollapseWindowSeconds` **И** подтверждён (drop/overflow-rate > 0 ИЛИ gap > `fpsCollapseGapMsThreshold`). baseline — скользящее среднее за `cameraBaselineWindowSeconds`, прогрев до `cameraBaselineSkipSeconds` пропускается | hard |

**Пороги** (константы `RecordingConfiguration.mvpDefault`; значения по умолчанию, не pass/fail-критерии):

| Константа | Значение | Что задаёт |
|---|---|---|
| `criticalSustainSeconds` | 10.0 с | Непрерывная длительность `.degraded` до live-hard `sustainedDrops` |
| `criticalDropRatePerMin` | 600 drops/мин | Порог нормализованной интенсивности дропов для post-stop hard |
| `criticalDropRateMinSessionSeconds` | 10.0 с | Пол длительности сессии: короче — post-stop никогда не срабатывает |
| `fpsCollapseRatio` | 0.5 | Доля baseline, ниже которой delivered-fps считается провалом |
| `fpsCollapseWindowSeconds` | 5.0 с | Непрерывная длительность провала до срабатывания |
| `fpsCollapseGapMsThreshold` | 250 мс | Порог максимального межкадрового gap как подтверждающего сигнала |
| `cameraBaselineWindowSeconds` | 30.0 с | Окно скользящего среднего baseline delivered-fps |
| `cameraBaselineSkipSeconds` | 2.0 с | Холодный старт: ранние чтения не кормят baseline |
| `criticalNotificationDedupeSeconds` | 10.0 с | Окно дедупликации live-уведомлений (первое в окне постится, повтор того же яруса подавляется) |

> **Калибровка порогов — L5 на MX Brio (обязательно перед релизом, ⏳ не выполнено).**
> Пороги выставлены теоретически и требуют проверки на реальном железе по AC-7:
> **затемнение/спад освещённости НЕ должны срабатывать**, реальный стопор (stall) —
> ДОЛЖЕН. Это прямо завязано на уже задокументированное поведение Brio (§3.1): под
> комнатным светом AE-droop опускает доставку до ~19–23 fps — это легитимный троттлинг
> *без* drop/overflow и без gap, поэтому AND-подтверждение детектора его НЕ ловит (dip без
> корроборации не срабатывает). Настоящий stall даёт gap'ы/дропы → срабатывает. L5 должен
> подтвердить, что `fpsCollapseRatio`/`fpsCollapseGapMsThreshold` разводят эти два случая на
> Brio, и что `criticalSustainSeconds`/`criticalDropRatePerMin` не дают ложных hard на
> нормальном контенте.
...
| 2026-06-22 | v0.5 | Добавлен §3.7 «Критические сигналы записи»: 3 условия «пожара» (потеря камеры soft/hard, устойчивые потери, коллапс fps), таблица 9 порогов `RecordingConfiguration.mvpDefault` со значениями по умолчанию, заметка об обязательной L5-калибровке на MX Brio по AC-7 (затемнение не срабатывает, stall срабатывает) со ссылкой на §3.1 |
