...
| Устройство | Режим | Статус поддержки |
|---|---|---|
| Logitech MX Brio | **1080p60** (автовыбор + ручной выбор) | ⏳ L5-верификация fps-lock ещё не проводилась (#113 Stage B) |
| Logitech MX Brio | ~~4K30/4K60~~ | ❌ **недоступно** — AVFoundation на macOS не доставляет заявленный Brio формат 3840×2160: AVCaptureSession опускает `activeFormat` до 1080p (нет пресета `.inputPriority` на macOS; подтверждено L5: оба пути — `setActiveFormat(4K)` и `.hd4K3840x2160`-пресет — дают 1080p, `activeFormat` читается обратно как 1080p). MJPEG также не экспонируется (CoreMediaIO декомпрессирует ниже AVFoundation). Режим 4K исключён из предлагаемых опций навсегда. См. [#113](https://github.com/kirich1409/Onset/issues/113). |
...
> **Состояние кода (#113 Stage B):** `CameraFormatSelector.pickBestFormat` предпочитает
...
> fps-равенстве предпочитается бо́льший fps (60 перед 30). `availableModes` явно ограничивает
> предлагаемые режимы ≤1080p — 4K исключён из выдачи энумератора. `CameraSource` использует
> `setActiveFormat` (прямое назначение формата) с отключением `isAutoVideoFrameRateEnabled`
> и DIAGNOSTIC-логированием (поиск: `DIAGNOSTIC #113` в os.log) для диагностики fps-lock.
> Ожидаемый результат L5: 1080p60 в выходном файле. Максимум камерного канала — **1080p60**.
...
**Extended-набор** — конфигурации с внешними устройствами (MX Brio). Выполняется при наличии устройства; **обязателен для релизов, затрагивающих capture**. Brio 4K30/4K60 исключены из матрицы — не поддерживаются AVFoundation на macOS (#113).
...
|---|---|---|
| 2026-06-08 | v0.4 | #113 Stage B: §2.1 — 4K недоступен (AVFoundation/USB/MJPEG), исключён из матрицы камер; §6.1 — удалены строки Brio 4K30; code-note обновлена (setActiveFormat + autoFR disable + DIAGNOSTIC #113); Extended-набор: 4K-оговорка заменена на факт |
