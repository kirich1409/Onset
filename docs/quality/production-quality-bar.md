...
|---|---|---|---|
| Потери кадров камеры — 43% номинала | [#112](https://github.com/kirich1409/Onset/issues/112) | ⏳ структурный дефект исправлен (#112, B-frames/gate); fresh-content приёмка при дневном свете — pending | Корень — B-frames (`AllowFrameReordering=true`): reorder window держал `NumberOfPendingFrames` ≥ 4, структурно достигая backpressure gate. Фикс: `allowFrameReordering=false` в `mvpDefault`. После фикса: gate_drop=0.00/с, пакетный rate=30.00 pkt/s (ассерт A PASS, gap_count=0). Fresh-content rate при дневном свете — ⏳ pending ре-верификации |
