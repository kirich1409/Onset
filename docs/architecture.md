...
| `GeneralPane` / `VideoPane` / `AudioPane` / `CameraPane` / `IndicationPane` | `UI/Settings/*Pane.swift` | Пять вкладок настроек. В v1 «Общие»/«Видео»/«Аудио» — read-only (язык/формат/шумодав/системный звук фиксированы, показаны статичными строками); «Камера» — тоггл зеркалирования камеры (`.nextRecordingStart`); «Индикация» (дефолтная вкладка) — тоггл таймера в меню-баре (`.immediate`) |
