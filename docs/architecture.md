...
| `MenuBarLabelMapper` | `UI/MenuBar/MenuBarLabelMapper.swift` | Чистый enum: phase+state+sourceLiveness → дескриптор лейбла меню-бара (красная/жёлтая точка, таймер, `deviceLostWarning`, accessibilityLabel). `deviceLostWarning` (#261) сигналит потерю активного устройства захвата (камера/микрофон/экран) во время записи независимо от `recordingState` — единственный видимый сигнал в menu-bar-only режиме без открытого окна записи |
