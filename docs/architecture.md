...
| `RecordingConfiguration` | `Configuration/RecordingConfiguration.swift` | Иммутабельная политика: HEVC-настройки, таблица VBR-битрейтов, границы fps, бюджет; флаг `cameraMirror` (горизонтальное зеркалирование камеры; дефолт `false` в `makeMVPDefault`, читается на record-шве); `diskThresholds: DiskThresholds` (#88) — пороги места как данные конфиг-слоя, не хардкод; `budgetCap: EngineBudgetCap` — device-budget сцепка (AC-Q9, Phase A), см. подраздел ниже |
| `ChipTier` | `Configuration/ChipTier.swift` | Чистый `nonisolated enum` (2 case: `.m3Max`, `.uncalibrated`) — AC-Q4-калибровочный ярус хостовой SoC; полная компиляторная синтеза `Equatable`/`CaseIterable` (spike-verified исключение из обычного правила explicit-witness на enum'ах, см. doc-комментарий типа) |
| `ChipTierDetector` | `Configuration/ChipTierDetector.swift` | `nonisolated enum`-неймспейс: чистый `chipTier(forBrandString:)` (парсинг) + нечистый `detectChipTier()` (`sysctlbyname("machdep.cpu.brand_string")`, `unsafe` C-interop); любой сбой чтения → `.uncalibrated`, никогда не крашит |
...
### Device-budget flow (ChipTier, Phase A — device-budget coupling, AC-Q9)

Цепочка, определяющая допустимый суммарный pixel-rate (экран + камера) в зависимости
от хостового чипа:

```
ChipTierDetector.detectChipTier()   — sysctlbyname("machdep.cpu.brand_string"), unsafe, never crashes
  → ChipTier                        — .m3Max | .uncalibrated (2 case, conservative by construction)
    → EngineBudgetCap.budgetCap(for:codec:) — exhaustive switch, per-tier throughput ceiling
      → RecordingConfiguration.makeDefault(chipTier:) — задаёт config.budgetCap
        → config.budgetCap.fits(screen:camera:)       — preflight-проверка перед стартом сессии
```

- **`.m3Max` — 622.08M px/s, validated floor, НЕ AC-Q4.** Число — это эмпирически
  наблюдённая 0-drop точка из L5 #281 (4K60 экран + 1080p60 камера), но снятая на
  неworst-case клипе (не худший контент, не тепловое плато, без сопутствующей
  нагрузки). Это годный нижний предел, но НЕ калиброванный потолок: AC-Q4 (полноценная
  worst-case L5-калибровка на MX Brio до теплового плато под нагрузкой) реализуется
  только успешным прогоном **T-6**. До T-6 AC-Q4 явно UNREALIZED, а константа `.m3Max`
  в `budgetCap(for:codec:)` (`RecordingPolicyTypes.swift`) несёт комментарий, что она
  сеяна из #281 и НЕ должна восприниматься как откалиброванная. T-6 может либо поднять
  `.m3Max` до calibrated ceiling (если `0.85 × worst 0-drop cliff ≥ 622.08M`), либо,
  в честном худшем исходе, оставить калиброванный потолок НИЖЕ 622.08M — тогда именно
  калиброванный потолок управляет, а сама 622.08M-комбинация из таксономии становится
  недоступной в Phase B.
- **`.uncalibrated` — 248.832M px/s, safe-low, никогда не наследует (AC-Q9).** Любой
  распознанный или нераспознанный чип, кроме M3 Max, получает этот консервативный
  потолок (одного устойчивого 4K30-эквивалентного потока) — строго ниже `.m3Max` при
  любом состоянии обеих констант (проверяется тестом по `allCases`, не литералом).
  Чип получает более высокий бюджет только через свою собственную AC-Q4-калибровку
  (#292); до тех пор он остаётся на safe-low, независимо от реальной мощности media
  engine (Pro/Max/Ultra — количество media-engine не выводимо из маркетингового
  суффикса, поэтому никакого per-engine множителя здесь нет).
- **Сверка с #262 (активный сигнал critical-recording, `main` b355e24).** Спека
  писалась до #262 и рамит preflight-бюджет как *единственную* защиту, а промах —
  как *тихо* плохой. Это устарело: #262 уже добавил активный сигнал (Time Sensitive
  уведомление на критических drop'ах/fps-коллапсе). Итоговая модель — две линии
  защиты: preflight budget-cap (`fits(screen:camera:)`) остаётся первичной защитой
  (не даёт стартовать заведомо неподъёмной комбинации), а `#262`-сигнал — вторая,
  **громкая** линия на случай, если бюджет всё же промахнулся (неверная калибровка,
  непредвиденная деградация под нагрузкой) — пользователь узнаёт немедленно, не
  постфактум по сломанной записи.
- **Startup wiring — Phase B, ещё не активно в рантайме.** `RecordingConfiguration
  .makeDefault(chipTier:)` — это готовый injection seam (принимает `ChipTier` снаружи),
  но ничто в `OnsetApp`/`RecordingCoordinator` пока не вызывает
  `ChipTierDetector.detectChipTier()` и не передаёт результат в `makeDefault`.
  Сегодня рантайм всё ещё строится через `RecordingConfiguration.mvpDefault`
  (995M px/s, см. ниже) — device-budget цепочка выше существует и покрыта тестами,
  но не подключена к старту записи до Phase B.

Где искать типы: `Configuration/ChipTier.swift`, `Configuration/ChipTierDetector.swift`,
`Configuration/RecordingPolicyTypes.swift` (`EngineBudgetCap.budgetCap(for:codec:)`),
`Configuration/RecordingConfiguration.swift` (`makeDefault(chipTier:)`).
