...
GOP-aware whole-drop (keyframe-окно в FileWriter) добавлен в #150.
...
## 9. Keyframe-aware GOP-drop окно в FileWriter (#150)

### Проблема до #150

При `isReadyForMoreMediaData == false` `FileWriter` отбрасывал ровно один сжатый HEVC-сэмпл
и продолжал принимать последующие. Если отброшенный сэмпл был дельта-кадром, оставшиеся
дельты текущего GOP ссылались на отсутствующего предшественника — декодер воспроизводил
рваный артефакт вплоть до следующего ключевого кадра (~1–2 с при стандартном
`keyFrameIntervalSeconds`). Потеря была молчаливой: DropEvent с `.encoderBackpressureDrops`
эмитировался, но не различал «рваный GOP» от «чистого пропуска».

### Решение (#150): флаг `droppingUntilKeyframe`

`FileWriter` хранит actor-изолированный флаг `private var droppingUntilKeyframe: Bool`.

**Таблица переходов** (D = окно активно, R = input ready, K = keyframe):

| D | R | K | Действие |
|---|---|---|----------|
| 0 | 1 | – | Append (штатный путь) |
| 0 | 0 | – | Открыть окно, дропнуть кадр |
| 1 | 1 | 1 | Закрыть окно, appended |
| 1 | 1 | 0 | Дропнуть (дельта в окне — re-tear недопустим) |
| 1 | 0 | – | Дропнуть (input не готов, включая keyframe) |

**Инварианты:**
- Окно открывается на первом not-ready сэмпле (любой тип — keyframe или delta).
- Окно закрывается **только** при одновременном выполнении R=1 и K=1.
- Дельта при R=1 внутри окна дропается — appending delta after a gap re-tears the GOP.
- На каждый дропнутый кадр эмитируется один `DropEvent(.encoderBackpressureDrops, source: .writer)`.
  Счётчик DropMonitor и degraded-состояние работают как прежде.
- `markFaulted` / `isFaulted` — отдельный путь, не пересекается с GOP-окном.

### Эффект

Выход файла содержит **чистый разрыв** (один или несколько целых отсутствующих GOP) вместо
рваного региона. Файл остаётся валидным и воспроизводимым; потеря ограничена завершёнными GOP.

### Верификация (L5)

`scripts/verify-cfr.sh` + `ffprobe` на реальной записи с провоцированным writer-backpressure
(MX Brio, `Onset-L5` test plan). Ожидаемый результат: (1) файл воспроизводится без артефактов;
(2) при наличии дропов — только чистые GOP-пробелы; (3) `DropMonitor.degraded` / счётчик
срабатывают при sustained дропах.

---

## 10. Writer fault — отдельная поверхность, не backpressure-drop (#105)
...
## 11. Pinning аудио-формата (`CameraSource.audioOutputSettings`) (#105)
...
## 12. Эмпирическое наблюдение: 0 дропов на тихой машине (2026-06-08, #100/#65)

> Контекст: измерение проводилось в рамках диагностической работы по issues #100 и #65.

**Условия:** запись 4K60 экрана + камера (MX Brio), машина оставлена в покое ~30–45 с после старта
записи, никаких действий на записываемом дисплее.

**Результат:** 0 dropped frames на протяжении всего прогона. Пайплайн encode/write не создаёт
дропов в установившемся режиме — при отсутствии нагрузки на записываемый дисплей overhead
отсутствует.

**Предыдущее измерение ~191 дропов за 64 с оказалось артефактом:** в том прогоне QA-инструмент
непрерывно делал полноэкранные снимки и accessibility-запросы на том самом 4K60-дисплее, который
записывался. Конкуренция за GPU/энкодер создала нагрузку, не типичную для реального использования.

**Вывод для #100/#65:** значимое измерение дропов требует репрезентативной активности на
записываемом дисплее — не idle (занижение → 0) и не QA-автоматизации с тяжёлыми снимками экрана
(завышение). Условия замера нужно фиксировать в результатах как неотъемлемую часть данных.
