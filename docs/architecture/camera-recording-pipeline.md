...
```
AVCaptureSession
  └─ CameraFormatSelector.pickBestFormat   // max-пиксели при maxFps≥30
...
`CameraFormatSelector.pickBestFormat` выбирает формат с максимальным числом
пикселей среди тех, у которых `maxFrameRate ≥ 30`. Это предпочитает разрешение
частоте для UVC-камер с поддержкой нескольких режимов (4K30 vs 1080p60).
...
`CameraFormatSelector` выбирает режим 4K30 перед 1080p60 (больше пикселей).
Выбор режима (4K30 / 1080p60 / другие) пользователем не реализован — issue #113.
