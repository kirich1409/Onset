...
индикатор menu bar — вторичный И запасной для **hard**, когда уведомления
запрещены/подавлены. Для **soft** (`cameraAndScreen`) октагона нет, пост-стоп-нота
тоже уведомление → при denied + без окна soft-инцидент остаётся **disk-only by
design** (низкая срочность, экран пишется штатно). Known-limitation: на Mac с нотчем
переполненный menu bar может прятать крайний status-item — гарантированный канал для
hard тогда тоже пост-стоп; учесть при L5-калибровке.
...
подавление (soft показан → пришёл hard → hard постится). Сверх per-window дедупа —
**session-level cap**: live-уведомление каждого тира постится максимум один раз за
сессию (на session max-severity latch). Так рецидив windowed-hard (drop-storm на 1-й
и на 30-й минуте) не выливается в серию прерываний Focus. Полную картину всех
...
- **Два разных значения на координаторе** (не путать — иначе индикатор «застрянет
  пожаром»): (а) **live critical view** — текущий вердикт детекторов,
  **де-эскалирует** при восстановлении windowed-hard; именно его `MenuBarLabelMapper`
  читает вторым входом для индикатора; терминальный `cameraOnly` в нём латчится до
  stop. (б) **session max-severity latch** (`criticalLatch`) — хранит max-severity
  увиденного за сессию, **только** для пост-стоп ветки, в live-вид не подаётся.
...
- **Live-seam fps-снимка (P1) — точный путь доставки**: `currentDrops()`
  ...
  **только** `FpsCollapseDetector` — это camera-capture fps/gap-снимок;
  ...
  - добавить параллельный pull-метод `currentRates()` (тот же 1 Hz tick), который
    ...
    источника (`rateLock.withLock` для capture — `CameraSourceShims.swift:50`),
  ...
  - snapshot несёт **возраст** (timestamp/tick-id последнего flush): при обрыве
    камеры flush прекращается, и детектор должен отбросить устаревший вход (сравнить
    с получаемым монотонным elapsed), а не трактовать заморозку как стабильный fps;
...
- AC-3 [L2]: `SustainedDropDetector` — degraded ≥ `criticalSustainSeconds` → hard;
  < порога → нет; live-уведомление **однократно за сессию** (рецидив после
  де-эскалации индикатор показывает, но второго Focus-баннера не шлёт); после
  восстановления вид де-эскалирует, session-латч для пост-стопа сохраняется.
...
- [частично закрыт; остаток → #339] Эмпирическая устойчивость
  baseline+freeze+drop/gap-подтверждения против всей легитимной вариативности камеры.
  **Cold-start ramp — проверен 2026-07-19**: запись 357 с с разгоном камеры с нуля, ни
  одного срабатывания детектора коллапса. Разрывы между кадрами в нормальном свете
  замерены: `gap_ms_avg` 34.5 мс, `gap_ms_max` 100 мс при пороге
  `fpsCollapseGapMsThreshold` 250 мс.
  **Low-light throttle — НЕ проверен**: требует физически затемнённой комнаты, софтом не
  воспроизводится (пять способов проверены и закрыты — см. #339). Расчётно троттлинг до
  8–20 fps даёт разрывы 50–125 мс, тоже ниже порога, поэтому ложного срабатывания не
  ожидается — но это теория, не замер.
  PR смержен до закрытия этого пункта решением владельца; пороги остаются гипотезой до
  верификации в #339.
- [non-blocking] Рендерится ли непрерывная пульсация (`symbolEffect`) в label
  MenuBarExtra status-item — известное ограничение. Различимость уже обеспечена
  внутренним глифом+цветом (AC-11), пульсация — бонус, не несущая; спайк ничего не
  блокирует.
