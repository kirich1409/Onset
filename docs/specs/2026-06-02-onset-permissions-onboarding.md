...
- [ ] **AC-6** — Запись экрана не имеет отдельного состояния «Запрещён» (**amended 2026-06-02**, см. ниже): macOS отдаёт только `CGPreflightScreenCaptureAccess() -> Bool`, и «отклонено» неотличимо от «ещё не запрошено». Карточка экрана имеет состояния «Требуется» → «Ожидание…» → ✓; путь «Открыть настройки» (+ пронумерованная ручная инструкция) доступен всегда, пока доступ не выдан; системный prompt не зацикливается (`CGRequestScreenCaptureAccess()` вызывается единожды, в т.ч. чтобы Onset появился в списке System Settings → Запись экрана). Опция «Продолжить без экрана» доступна всегда, когда экран не выдан и доступна камера. *(Камера/Микрофон сохраняют реальное denied-состояние через `AVAuthorizationStatus.denied` — карточка «Запрещён» + «Открыть настройки».)*
- [ ] **AC-7** — Graceful-варианты работают: «Продолжить без экрана» → переход к записи только камеры + звука (доступна всегда, когда экран не выдан и есть камера, не только в denied-ветке); «Записать без звука» (микрофон не выдан) → переход к записи без аудио-дорожки. Кнопка перехода к записи активна при наличии достаточного подмножества разрешений — в т.ч. в состоянии ожидания экрана, если уже выдано достаточное подмножество (камера).
