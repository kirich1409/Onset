...
- [x] **AC-10** — ✅ Закрыт L5-прогоном 2026-07-19 (14 записей на M3 Max / MX Brio, подписанная сборка; отчёт `docs/plans/disk-space-management/calibration-l5.md`): (а) PASS — два авто-стопа (по полу и по ETA), оба файла валидны, Δ длительностей 0.28 и 0.66 с при `movieFragmentInterval` 4 с; (б) PASS — требуется ≈41 МБ против порога 2000 МБ; (в) **ЧАСТИЧНО** — purgeable измерен (34 ГБ оптимизма на системном томе), но поведение ОС при заполнении *реального* тома с purgeable не воспроизведено, остаточный пробел; (г) PASS по жёсткому критерию — стоимость на main ниже порога семплирования (1 семпл из ~60 000), блокирующие чтения на выделенной очереди, латентность 10 мс медиана / 23 мс max, каденция 3.9 с, зависаний нет ни в одном прогоне; дельта по `encoderBackpressureDrops` **недостоверна** (n=6 пар, разброс внутри плеча 14×, знак переворачивается) — причинного механизма не найдено; (д) PASS — сырой SNR 0.91 < отсечки 1.0, EWMA 16 с даёт 2.84. Пороги изменений не потребовали. Ниже — исходная формулировка критерия. Пороги — reasoned defaults (гипотеза), откалиброваны эмпирически. L5-прогон: том вывода направляется на size-capped APFS-том / sparse disk image с квотой, заполняется до нужного остатка (изолирует от системного тома). Подтвердить: (а) штатная остановка с валидными файлами; (б) реальное время финализации двух файлов укладывается в критический запас 2ГБ; (в) поведение ОС/purgeable вблизи заполнения не ломает оценку; (г) перф-бюджет мониторинга (AC-2) — измеримая дельта, НЕ «нет фризов на глаз»: под representative-load сравнить прогон с включённым и выключенным disk-мониторингом — критерий «нет прироста `encoderBackpressureDrops` и нет hang > 250 мс» (Instruments Hangs / hang-tracking). Расхождения → правка порогов до merge.
...
| Пороги (числа) | Сист. warn ≤10ГБ/стоп ≤5ГБ; вывод warn ETA≤10мин\|≤10ГБ/стоп ≤2ГБ\|ETA≤2мин | **Откалиброваны L5 2026-07-19 — изменений не потребовалось** (`docs/plans/disk-space-management/calibration-l5.md`). Запас 2ГБ избыточен ~в 48× для скорости записи, но необходим против внешнего потребления: при 310 МБ/с пол выгорает за 6.5 с — быстрее одного `readEvery`, и остановку обеспечивает ETA-ветка. `ewmaTimeConstantSeconds` понижать нельзя: на каденции 4 с сырой SNR наклона = 0.91 < отсечки 1.0, сглаживание 16 с даёт 2.84. Порог системного тома сравнивается с оптимистичным `...ImportantUsage` — на реальном SSD замерено 34 ГБ purgeable-оптимизма, кратно превышающих сами пороги |
