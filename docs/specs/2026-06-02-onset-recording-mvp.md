...
- [ ] **AC-1** — Главный экран показывает три селектора: Дисплей (если дисплеев > 1; иначе единственный выбран по умолчанию), Камера (устройство) и Микрофон (устройство), плюс live-превью выбранной камеры, и секцию «ВЫВОД» с выбором базовой папки (#225). Настроек кодека/контейнера/разрешения на экране нет.
- [ ] **AC-2** — Активация кнопки «Записать»: (а) экран является обязательным видео-источником MVP — кнопка активна когда доступ к экрану выдан и дисплей выбран (камера опциональна); (б) если доступный микрофон есть, но не выбран в пикере — кнопка disabled + подсказка «Выберите аудио-вход, чтобы начать запись» (макет пустого состояния); (в) если микрофон недоступен (нет устройства / нет разрешения) — кнопка активна, запись идёт без аудио, рядом индикатор «без звука». Отсутствие микрофона НЕ блокирует запись (согласовано с graceful «Записать без звука» онбординга).
- [ ] **AC-3** — По «Записать» стартует запись: главное окно скрывается, menu bar переходит в состояние Recording (● + таймер). Окно записи (статус «● ИДЁТ ЗАПИСЬ», таймер, чек-лист источников, кнопка «Остановить») открывается **по требованию** через «Открыть окно записи» в menu bar (#242 — menu-bar-first). Запуск сопровождается локальным уведомлением «Запись началась» (если разрешено; тихий fallback при отказе).
- [ ] **AC-4** — Записываются **два файла** в подпапку сессии `<базовая папка>/Onset YYYY-MM-DD HH.mm.ss/` (дефолт базовой папки — `~/Movies/Onset/`; пользователь выбирает её в секции «ВЫВОД» главного экрана): экран и камера, кодек **HEVC Main 8-bit**, контейнер **.mp4 (hvc1)**, **CFR**. Звук выбранного микрофона пишется в **оба** файла. При коллизии (та же секунда) к имени подпапки добавляется суффикс ` (N)`.
- [ ] **AC-5** — Экран пишется с разрешением выбранного дисплея, **но не выше cap по бюджету encode-движка** (CapabilityProbe; дефолт ≤ 4K60 — дисплеи 5K/6K downscale до вписывания; _5K/6K — граничная защита на гипотетические дисплеи, MVP-цель — 4K_), CFR (целевой fps = **min(нативная частота обновления дисплея, 60)**: дисплей ≤60 Гц → его частота; 120 Гц → 60, кратным понижением; >60 — post-MVP). Камера — авто-выбранный **наибольший 16:9 формат** (record path выбирает наибольшее доступное разрешение — native 4K, когда камера его отдаёт; 60fps остаётся недостижим на AVFoundation-стеке — hardware-constraint конкретных камер, см. [`macos-avfoundation-camera-limits.md`](../quality/macos-avfoundation-camera-limits.md)); ручного выбора режима нет. CFR на своём fps. Разные fps экрана и камеры допустимы и пишутся независимо.
...
- [~] **AC-8** — Пропущенные кадры считаются **раздельно по причинам** (backpressure энкодера/диска vs late-кадр камеры; отсутствие нового кадра статичного экрана в SCStream НЕ считается дропом). ~~и отображаются в окне записи~~ Живой счётчик дропов в окне записи **удалён** (решение пользователя): потери кадров пишутся в per-session технический отчёт на диске (см. AC-9), а в окне остаётся только индикатор Degraded. Degraded (🟡⚠ + таймер) включается при backpressure-дропах > порога за скользящее окно T секунд (порог и T — параметры конфигурации, калибруются post-MVP; единичный исторический дроп не деградирует). _Частично (#100): `DropCause` per-source тали + `sessionEverDegraded` latch реализованы; индикатор деградации сохранён, drop-pill удалён._
- [x] **AC-9** — Остановка доступна тремя путями: кнопка «Остановить» в окне записи, глобальный hotkey ⌘⌥⌃R (виден в menu bar как подсказка рядом с «Остановить»), действие «Остановить» из menu bar. По остановке оба файла финализируются и становятся валидными; пользователю показывается результат (reveal в Finder — открывает **папку сессии**). ~~Если за сессию были существенные backpressure-дропы — результат несёт предупреждение («запись завершена, пропущено N кадров — возможны рывки»).~~ Post-stop предупреждение о пропущенных кадрах **удалено** (решение пользователя). Вместо него каждая сессия с выводом пишет per-session технический отчёт `Onset YYYY-MM-DD HH.mm.ss — Техническая информация.txt` в папку сессии рядом с файлами записи (счётчики `DropCounters`, разбивка `DropBreakdown`, факт деградации, основная причина). Алерт об **ошибке записи файла** (write-error) сохранён. _Реализовано: reveal открывает папку (#225); ⌘⌥⌃R hotkey показан в menu bar (#242); технический отчёт — `DropReportFormatter` + `RecordingOutput.writeReport`._
...
- [ ] **AC-11** — Graceful по разрешениям: без микрофона — файлы пишутся без аудио-дорожки; без камеры — пишется только файл экрана. Невозможность записать экран (нет разрешения) блокирует старт (MVP: экран обязателен). _Запись только с камерой (без экрана) — деferred post-MVP (решение B, issue #61)._
...
| Базовая папка вывода | ✅ Done | Agent | Дефолт `~/Movies/Onset/`; пользователь меняет в секции «ВЫВОД»; подпапка сессии создаётся на старте записи (#225) |
...
| `Storage/RecordingOutput` | New | пути/имена файлов; reveal в Finder (папки сессии); (опц.) метаданные через SwiftData |
| `Storage/OutputFolderStore` | New (#225) | Персистирование базовой папки вывода в UserDefaults (`OutputFolderPersisting`) |
| `Storage/OutputDirectoryNaming` | New (#225) | Чистые утилиты: имя подпапки сессии (`Onset YYYY-MM-DD HH.mm.ss`), collision avoidance, валидация базовой папки |
| `Configuration/OutputFolderKeys` | New (#225) | UserDefaults-ключ для базовой папки |
| `UI/Main/MainView+Sections` (outputSection) | New (#225) | Секция «ВЫВОД» в главном окне: строка «Папка» + `NSOpenPanel`-пикер |
...
|---|---|
| Секция ЭКРАН: тумблер «Запись экрана» | **Удалено** (MVP: экран обязателен, тумблер нефункционален; решение B, issue #61) |
...
| Секция ВЫВОД: Кодек, Контейнер, warning | **Удалена целиком** (HEVC/.mp4 фикс; Phase 2) |
| Секция ВЫВОД: Папка | **Остаётся** — строка «Папка» с выбором базовой папки через `NSOpenPanel`; персистируется в UserDefaults (#225) |
...
| Экран: denied (нет разрешения) | — | пустое состояние «Запись недоступна — выдайте разрешения» + кнопка возврата в онбординг (MVP: запись без экрана deferred post-MVP, issue #61) |

### Захват
- **Экран:** `SCStream` с `SCStreamConfiguration`: `width/height` = разрешение выбранного дисплея (с учётом cap из pre-flight, см. ниже); `minimumFrameInterval = CMTime(1, targetFps)`, где `targetFps = min(нативная частота дисплея, 60)` — частота дисплея определяется на старте (зависит от разрешения), 120 Гц пишется как 60; подходящий `pixelFormat` (8-bit 4:2:0, напр. `420v`); `queueDepth` достаточный для backpressure. `SCContentFilter` = весь дисплей, исключая собственные окна Onset по app-identity (#244) — on-demand окно записи не попадает в захват. `SCStreamOutput` отдаёт `CMSampleBuffer` (IOSurface-backed) на выделенной `sampleHandlerQueue`.
- **Камера:** `AVCaptureSession` (НЕ MultiCam) с выбранным `AVCaptureDevice`; `activeFormat` подбирается авто-эвристикой: предпочитается **наибольший 16:9-формат** с fps≥30. Record path (`allowAboveFullHD: true`) снимает кэп по высоте — выбирается наибольший доступный 16:9-формат (native 4K, когда камера его отдаёт); preview/device-list путь (`allowAboveFullHD: false`, дефолт) остаётся кэпнут на ≤1080p. Если все 16:9-форматы выше кэпа — берётся наименьший из них (preview-путь) или наибольший (record-путь); при отсутствии 16:9-форматов — фолбэк на наибольшее по пикселям. Ручного пикера режима в MVP нет; 60fps недостижим через AVFoundation на macOS — hardware-constraint конкретных камер, не лимит стека (отслеживается в [#178](https://github.com/kirich1409/Onset/issues/178)). `AVCaptureVideoDataOutput` → `CVPixelBuffer` на своей очереди.
...
Pre-flight даёт защиту от сценария «M1 Air + внешний 5K-монитор → непрерывный backpressure → час брака»: на слабом чипе с большим дисплеем запись стартует уже в выполнимом разрешении, а не пишет брак под молчаливым Degraded. _(5K-монитор здесь — пример граничной нагрузки; MVP-цель — 4K-дисплеи.)_

### Запись в файл
`AVAssetWriter` (fileType `.mp4`, codec tag `hvc1`) с video `AVAssetWriterInput` (или `AVAssetWriterInputPixelBufferAdaptor`) + audio `AVAssetWriterInput`. **`movieFragmentInterval`** задан (устойчивость к крашу). Структура вывода (#225):
- Базовая папка: `~/Movies/Onset/` по умолчанию; пользователь выбирает через `NSOpenPanel` в секции «ВЫВОД» главного экрана; сохраняется в `UserDefaults` (`OutputFolderStore`).
- На старте каждой сессии создаётся подпапка: `<базовая>/Onset YYYY-MM-DD HH.mm.ss/` (`OutputDirectoryNaming`). При коллизии — суффикс ` (N)` на уровне подпапки (не файла).
- Имена файлов внутри подпапки: `Onset YYYY-MM-DD HH.mm.ss — Screen.mp4` и `Onset YYYY-MM-DD HH.mm.ss — Camera.mp4` (единый timestamp на сессию записи).
- Если базовая папка не существует или не записываема на старте записи — запись не стартует, пользователю показывается алерт.
- После остановки «Показать в Finder» открывает **папку сессии** (не выделяет отдельные файлы).
...
- **Порядок старта.** Подготовить оба пайплайна (writer'ы в состоянии `.writing`, `startSession(atSourceTime: T0)` вызван) ДО старта захвата; затем стартовать источники (`SCStream.startCapture()` и `AVCaptureSession.startRunning()` — порядок между ними не важен, т.к. эпоха уже зафиксирована). Кадры с host-time < T0 отбрасываются. **UI-гейт (#171): на macOS 26 `SCStream.startCapture()` возвращается до того, как пользователь подтверждает consent-диалог.** Переход в состояние «идёт запись», открытие recording window и старт elapsed-таймера происходят только после того, как первый реальный кадр экрана пришёл из ScreenCaptureKit — это сигнал о выданном consent. При отказе (consent denied, терминальный стоп до первого кадра) или превышении таймаута (~30 с) сессия останавливается, бросается `RecordingError.captureDidNotActivate`.
...
MVP: Onset — **обычное приложение с иконкой в Dock** (`MenuBarExtra` + окна). Полный режим menu-bar-accessory (LSUIElement, без Dock, вход только через menu bar) отложен post-MVP (#63).
...
- **Клик по иконке в Dock во время записи** — фокусирует окно записи (`WindowID.recording`), а не главное окно. Обрабатывается `applicationShouldHandleReopen` через `RecordingCoordinator.handleReopen()` (#272).
- **Переход в запись (#242 — menu-bar-first).** По «Записать» главное окно **скрывается**; окно записи (`RecordingView`) **не открывается автоматически** — доступно по требованию через «Открыть окно записи» в menu bar. Запуск подтверждается локальным уведомлением «Запись началась». Источники во время записи менять нельзя — окно записи показывает read-only чек-лист.
...
- **Красная кнопка в title bar окна записи** — **скрывает** окно (не останавливает запись, #242). Graceful Cmd-Q с ожиданием финализации — #243.
...
| Папка | Базовая папка выбирается пользователем (дефолт `~/Movies/Onset/`); каждая сессия — отдельная подпапка `Onset <timestamp>/` (#225) | Пользователь управляет местом хранения; подпапка изолирует файлы одной сессии |
| Остановка | Кнопка + hotkey ⌘⌥⌃R + menu bar | Все три в макетах |
| Превью камеры | В MVP | В макете; дёшево (`AVCaptureVideoPreviewLayer`) |
| Активация «Записать» | Экран обязателен (выбранный дисплей); камера опциональна; микрофон не обязателен | Решение B (issue #61): camera-only deferred post-MVP; согласовано с graceful «Записать без звука»; различать «не выбран» vs «недоступен» (AC-2) |
...
- Меню настроек, выбор кодека/контейнера/разрешения/fps — *(Phase 2)*
...
- [x] **РЕШЕНО** Эвристика авто-выбора формата камеры (#145): дефолт — предпочитать 16:9, целевое разрешение Full HD (≤1080p): берётся наибольший 16:9-формат с высотой ≤1080 и fps≥30 (1080p при поддержке, иначе ниже — 720p и т.д.); если все 16:9-форматы выше 1080p — наименьший из них (ближайший сверху); при отсутствии 16:9-форматов — фолбэк на наибольшее по пикселям (tie-break: больший fps). Поверх дефолта — ручной выбор (#113). Явный пользовательский выбор формата — post-MVP (Phase 2, настройки).
...
|---|---|---|
| Запись только с камерой (без экрана) | Deferred — решение B: в MVP экран обязателен; `RecordingRequest.display` non-optional, `RecordingSession` не имеет screen-skip ветки | issue #61 |
| Исключение экрана пользователем («Без экрана») | Deferred вместе с camera-only путём | issue #61 |
