...
- **Камера:** `AVCaptureSession` (НЕ MultiCam) с выбранным `AVCaptureDevice`; `activeFormat` подбирается авто-эвристикой: предпочитается 16:9, целевое разрешение Full HD (≤1080p) — берётся наибольший 16:9-формат с высотой ≤1080 и fps≥30 (1080p при поддержке, иначе ниже — 720p и т.д.); если все 16:9-форматы выше 1080p — берётся наименьший из них; при отсутствии 16:9-форматов — фолбэк на наибольшее по пикселям. Поверх дефолта — ручной выбор (Phase 2, #113). `AVCaptureVideoDataOutput` → `CVPixelBuffer` на своей очереди.
