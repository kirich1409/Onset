...
- [ ] **AC-5** — Экран пишется с разрешением выбранного дисплея, **но не выше cap по бюджету encode-движка** (CapabilityProbe; дефолт ≤ 4K60 — дисплеи 5K/6K downscale до вписывания; _5K/6K — граничная защита на гипотетические дисплеи, MVP-цель — 4K_), CFR (целевой fps = **min(нативная частота обновления дисплея, 60)**: дисплей ≤60 Гц → его частота; 120 Гц → 60, кратным понижением; >60 — post-MVP). Камера — авто-выбранный **наибольший 16:9 формат** (макс. 1080p на AVFoundation-стеке; 4K30/1080p60 недостижимы через AVFoundation — см. [`macos-avfoundation-camera-limits.md`](../quality/macos-avfoundation-camera-limits.md)); ручного выбора режима нет. CFR на своём fps. Разные fps экрана и камеры допустимы и пишутся независимо.
...
- **Камера:** `AVCaptureSession` (НЕ MultiCam) с выбранным `AVCaptureDevice`; `activeFormat` подбирается авто-эвристикой: предпочитается **наибольший 16:9-формат** с высотой ≤1080 и fps≥30 (1080p при поддержке, иначе ниже — 720p и т.д.); если все 16:9-форматы выше 1080p — берётся наименьший из них; при отсутствии 16:9-форматов — фолбэк на наибольшее по пикселям. Ручного пикера режима в MVP нет; 4K30 и 1080p60 недостижимы через AVFoundation на macOS (вынесены в [#177](https://github.com/kirich1409/Onset/issues/177)/[#178](https://github.com/kirich1409/Onset/issues/178)). `AVCaptureVideoDataOutput` → `CVPixelBuffer` на своей очереди.
